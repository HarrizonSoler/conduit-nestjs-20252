...
import { EntityManager, EntityRepository, FilterQuery, QueryOrder, wrap } from '@mikro-orm/sqlite';
import { InjectRepository } from '@mikro-orm/nestjs'

import { User } from '../user/user.entity';
import { Article } from './article.entity';
import { ArticleResponse, ArticlesResponse, CommentsResponse } from './article.interface';
...
export class ArticleService {
  ...
  async findFeed(userId: number, query: any): Promise<ArticlesResponse> {
    const user = userId ? await this.userRepository.findOneOrFail(userId, { populate: ['followers', 'favorites'] }) : undefined;
  ...
  async findOne(userId: number, where: FilterQuery<Article>): Promise<ArticleResponse> {
    const user = userId ? await this.userRepository.findOneOrFail(userId, { populate: ['followers', 'favorites'] }) : undefined;
  ...
  async deleteComment(userId: number, slug: string, id: number): Promise<ArticleResponse> {
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
  ...
  async favorite(id: number, slug: string): Promise<ArticleResponse> {
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
  ...
  async unFavorite(id: number, slug: string): Promise<ArticleResponse> {
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
  ...
  async findComments(slug: string): Promise<CommentsResponse> {
  ...
  async update(userId: number, slug: string, articleData: any): Promise<ArticleResponse> {
    const user = await this.userRepository.findOneOrFail(userId, { populate: ['followers', 'favorites', 'articles'] });
    const article = await this.articleRepository.findOneOrFail({ slug }, { populate: ['author'] });
